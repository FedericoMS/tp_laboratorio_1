...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if(this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this)) //pElement PUEDE ser null, lo que no puede ser null es la lista//Empezó a funcionar mejor con el <=
    {
        ...
        if(nodeIndex == 0) //Caso donde se quiere enganchar el nodo como primer elemento de la lista
        {
            ...
            else //Caso donde la lista ya poseía nodos enganchados
            {
...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this != NULL && index >= 0 && ll_len(this) > index)
    {
        ...
        if(nodoActual != NULL)
        {
            ...
            if(next != NULL && index != 0)//MODIFIQUÉ ESTE IF PARA QUE NO ME TIRE ERROR CON EL LL_CONTAINSALL
            {
    ...
    if(this != NULL && this2 != NULL)
    {
        ...
        if(size1 >= size2)
        {
            ...
            for(i=0; i<size2; i++)
            {
                ...
                if(ll_contains(this, pElementAuxiliar) == 0) //tiene que ser igual a 0, así una vez que encuentra uno que no esté contenido, corta el for y no sigue iterando al pedo
                {
                    returnAux = 0;
                    break;
                }
